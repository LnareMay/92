...
public interface TransferHistoryRepository extends JpaRepository<TransferHistory, String> {
    ...
    @Query("SELECT th FROM TransferHistory th JOIN FETCH th.senderId JOIN FETCH th.receiverId WHERE th.senderId.memId = :memId order by transferDate desc")
    List<TransferHistory> findByMemId(@Param("memId") String memId);
    
    @Query("SELECT th FROM TransferHistory th JOIN FETCH th.senderId JOIN FETCH th.receiverId WHERE th.receiverId.memId = :memId")

...
public interface ClubMemberRepository extends JpaRepository<Club_Member_List, ClubMemberKeyClass> {
    ...
    @Query("SELECT COALESCE(count(*),0) FROM Club_Member_List cm WHERE cm.deleteFlag = false AND cm.clubCode = :clubCode")
    Integer countByClubCode(@Param("clubCode") String clubCode);
